...
def start_flower():
    """
    NeoPixel LED の初期化とデモ点灯
    """
    strip = init()
    # TODO: 最初にデモ点灯を行う。
    # TODO: LED 1個だけ動作確認用のLEDにする？色でステータス表示とか
    return strip


def output_flower(strip, emotions):
    # LED データを作成して、NeoPixel を制御します。
    # TODO: emotions は前バージョンでは PIPE から取得したデータをデコードして得ていた。データの書式については再検討
    # TODO: 表情の認識次第でチカチカする→主要な感情のだけ色を混ぜて表示するとグラデーション的に柔らかく遷移する？一定割合以上Happyになったら黄色前面に出すとか
